...
- 자동 생성 (@GeneratedValue)
  - IDENTITY - 데이터베이스에 위임. MySQL
    ...
    - mysql DB 컬럼의 경우 컬럼 생성시 not null auto_increments 추가
    - h2 DB 컬럼의 경우 컬럼 생성시 generated by default as identity
    - 직접 값을 넣어주지 않으므로 객체를 저장하고 나서야 ID값을 알수 있음.
    - 이 경우만 예외적으로 em.persist(obj)를 수행한다.
...
- initialValue - DDL 생성시에만 사용됨, 시퀀스 DDL을 생성할 때 처음 시작하는 수를 지정
...
키 생성 전용 테이블을 만들어서 시퀀스를 흉내냄

모든 데이터베이스에서 사용할수 있으나 성능 이슈가 발생할수 있음
